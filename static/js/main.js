...
document.addEventListener("DOMContentLoaded", function () {
  // Add event listeners to quantity input boxes
  var quantityBoxes = document.querySelectorAll(".cart-quantity-box");
  ...
  var incrementButtons = document.querySelectorAll(".inc.qtybutton");
  var decrementButtons = document.querySelectorAll(".dec.qtybutton");

  incrementButtons.forEach(function (button) {
    button.addEventListener("click", function () {
      var quantityBox = this.closest("tr").querySelector(".cart-quantity-box");
      if (quantityBox === null) {
        console.log("quantityBox is null");
      } else {
        quantityBox.value = parseInt(quantityBox.value) + 1;
        updateSubtotal(quantityBox);
      }
    });
  });
  ...
  decrementButtons.forEach(function (button) {
    button.addEventListener("click", function () {
      ...
      if (quantityBox === null) {
        console.log("quantityBox is null");
      } else if (parseInt(quantityBox.value) > 1) {
        quantityBox.value = parseInt(quantityBox.value) - 1;
        updateSubtotal(quantityBox);
      }
    });
  ...
  function updateSubtotal(quantityBox) {
    var quantity = parseInt(quantityBox.value);
    var price = parseInt(
      quantityBox.parentNode.parentNode
        .querySelector(".uren-product-price .amount")
        .innerText.replace("Ksh. ", "")
    );
    var subtotal = quantity * price;
    quantityBox.parentNode.parentNode.querySelector(
      ".product-subtotal .amount .subtotal"
    ).innerText = subtotal;
  }
});
